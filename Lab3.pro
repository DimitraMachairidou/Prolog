...
twice(C,N) :- hosted(I,Y,C),
			  hosted(J,X,C),
			  dif(I,J),
			  abc((Y-X),N).

in2Cities(D) :- hosted(I,Y,C),
				hosted(J,X,A),
				dif(I,J),
				dif(Y,X),
				dif(C,A),
...
move(A,B,C) :- nextStation(L,A,B),
			   entranceFare(L,F),
			   travelFare(L,D),
			   J=(F+D),
			   C>=J.
			   
move(A,B,C) :- nextStation(L,A,X),
			   entranceFare(L,F),
			   travelFare(L,D),
			   E=(F+D),
			   C>0,
